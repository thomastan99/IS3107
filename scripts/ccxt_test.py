...
import pandas as pd
import requests
from scripts.helper_functions import flatten_nested_json_df
...
def extract_coincap_api(dag_path):
    ...
    print(top_10_coins)
    top_10_coins.to_excel(f"{dag_path}/assets/sampledata_assets.xlsx")
    success_list = []
    error_list = []
    for coin in top_10_coins["data.name"]:
        ...
        try:
            ...
            results = flatten_nested_json_df(results)
            results.to_excel(f"{dag_path}/assets/sampledata_{coin}.xlsx")
            print(results.head())
